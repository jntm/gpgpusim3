...
kernel_info_t::increment_cta_id(unsigned core_id) 
{
  ...
  if (!m_to_be_switched.empty() && (m_to_be_switched.top().ready_time <= get_curr_cycle())) {
    ...
    kain_m_next_cta[core_id].pop_front();
  }
  m_next_tid.x=0;
  m_next_tid.y=0;
  m_next_tid.z=0;
...
/*
void
kernel_info_t::increment_cta_id()
{
  // First, check if any context switching CTA
  // Second, check if any context flushed CTA
  // Third, proceed normally
  if(m_next_cta.x < m_grid_dim.x && m_next_cta.y < m_grid_dim.y && m_next_cta.z < m_grid_dim.z)
  {
    increment_x_then_y_then_z(m_next_cta,m_grid_dim);
  }
  else if (!m_to_be_switched.empty() && (m_to_be_switched.top().ready_time <= get_curr_cycle())) {
    m_to_be_switched.pop();
  } else if (!m_to_be_migrated.empty()) {
    m_to_be_migrated.pop_back();
  } else {
...
long long kain_warp_inst_app3 = 0;
long long kain_warp_inst_app4 = 0;
void
kernel_info_t::inc_num_simulated_insts(unsigned active_count)
{
  ...
  if(get_kain_stream_id() == 3)
  		kain_warp_inst_app3 += 1;
  if(get_kain_stream_id() == 4)
  		kain_warp_inst_app4 += 1;
...
kernel_info_t::get_next_cta_id(unsigned core_id) const
{
  if (!m_to_be_switched.empty() && (m_to_be_switched.top().ready_time <= get_curr_cycle())) {
    ...
    assert(core_id< 128);
    return kain_m_next_cta[core_id].front();//2 is the SMs per cluster
  }
}

/*
dim3
kernel_info_t::get_next_cta_id() const
{
  if(m_next_cta.x < m_grid_dim.x && m_next_cta.y < m_grid_dim.y && m_next_cta.z < m_grid_dim.z)
  {
    return m_next_cta;
...
kernel_info_t::can_issue_new_cta(unsigned core_id) const
{
    ...
    if(kain_m_next_cta[core_id].size() != 0)
         can_issue = true;
