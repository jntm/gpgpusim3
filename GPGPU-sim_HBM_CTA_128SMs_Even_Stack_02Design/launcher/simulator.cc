...
//////////////////KAIN profiling
  ...
  if(stream->get_uid() == 1)
  {
        if(KAIN_kernel1 != kname) 
        {
            KAIN_kernel1 = kname; 
//            printf("Repartion %d, New kernel1, repartition\n", KAIN_Re_partition);
//            KAIN_Re_partition = true;
  ...
  if(stream->get_uid() == 2)
  {
        if(KAIN_kernel2 != kname) 
        {
            KAIN_kernel2 = kname; 
//            printf("Repartion %d, New kernel2, repartition\n", KAIN_Re_partition);
//            KAIN_Re_partition = true;
        }
  }
/*
...
unsigned long long kain_address[100];
unsigned long long kain_real_address[100];
long kain_app[100];
unsigned long long kain_size[100];
int kain_allocate_count;
...
Simulator::process_message(ChildProcess* process)
{
  ...
  printf("KAIN: ID %d, get message and process it\n",process->getID());
  fflush(stdout);
  switch (process->get_message_header()) {
    case MESSAGE_GPU_MALLOC:
    {
      ...
      void *tmp=cudaMalloc(size);
      unsigned app_id = process->getID(); 
      assert(app_id < 4);
      assert(kain_allocate_count < 100);
      kain_address[kain_allocate_count]=(long)tmp;
      kain_app[kain_allocate_count] = app_id;
      size_t real_size;
      if(size % 256)
        real_size = size +  (256 - size%256);
      else
        real_size = size;
      kain_size[kain_allocate_count] = real_size;

      long kain_real_address_tmp = (long)tmp;
      for(int i = 0; i < kain_allocate_count; i++)
      {
        if(kain_app[i] != app_id)
            kain_real_address_tmp = kain_real_address_tmp - kain_size[i];
      }
      kain_real_address[kain_allocate_count] = kain_real_address_tmp;
      printf("ID %d, KKKKKKKKKKKKKKKK tmp %0x, real address %0x\n", process->getID(), tmp, kain_real_address_tmp);

      kain_allocate_count++;

      for(int i = 0; i < kain_allocate_count; i++)
        printf("ID %d KAIN real address %0x, address %0x\n", kain_app[i],kain_real_address[i], kain_address[i]);
