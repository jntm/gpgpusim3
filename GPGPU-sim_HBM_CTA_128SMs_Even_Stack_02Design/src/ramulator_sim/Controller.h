...
extern unsigned long long kain_cycles_HBM_app1;
extern unsigned long long kain_cycles_HBM_app2;
extern unsigned long long kain_cycles_HBM_app3;
extern unsigned long long kain_cycles_HBM_app4;

extern unsigned long long kain_cycles_HBM_total;
extern unsigned long long kain_write_back_cycles;


extern unsigned long long kain_row_hit_app1;
extern unsigned long long kain_row_hit_app2;
extern unsigned long long kain_row_hit_app3;
extern unsigned long long kain_row_hit_app4;
extern unsigned long long kain_row_miss_app1;
extern unsigned long long kain_row_miss_app2;
extern unsigned long long kain_row_miss_app3;
extern unsigned long long kain_row_miss_app4;
...
protected:
    ...
    long kain_cycle_used;
    long kain_cycle_used_V2;
...
public:
    ...
    struct Queue {
        list<Request> q;
        unsigned int max = 64;//old 32 KAIN
    ...
    void finish(long read_req, long dram_cycles) {
        ...
        long KAIN_read = read_transaction_bytes.value();
        long KAIN_write = write_transaction_bytes.value();
    //    printf("KAIN_total read byte %ld, total write byte %ld, total cycles %ld, used cycles %ld\n",KAIN_read, KAIN_write, dram_cycles, kain_cycle_used);
        printf("kain cycle %lld, dram_cycle %lld, KAIN_total used cycles %lf\n",kain_cycle_used,dram_cycles,(float)kain_cycle_used/(float)dram_cycles);
        printf("kain V2 cycle %lld, dram_cycle %lld, KAIN_total used cycles %lf\n",kain_cycle_used_V2,dram_cycles,(float)kain_cycle_used_V2/(float)dram_cycles);
        channel->finish(dram_cycles);
    }

    long kain_total_used_cycle()
    {
        return kain_cycle_used;
    }

    long kain_total_byte()
    {
        long KAIN_read = read_transaction_bytes.value(); 
        long KAIN_write = write_transaction_bytes.value();
        return KAIN_read+KAIN_write;
    }

    long kain_total_byte_READ()
    {
        long KAIN_read = read_transaction_bytes.value(); 
        return KAIN_read;
    }

    long kain_total_byte_WRITE()
    {
        long KAIN_write = write_transaction_bytes.value();
        return KAIN_write;
    }
    ...
    long kain_total_row_access()
    {
       long KAIN_row_hit = row_hits.value();
       long KAIN_row_miss = row_misses.value();
       long KAIN_row_conflicts = row_conflicts.value();
       return KAIN_row_hit+KAIN_row_miss+KAIN_row_conflicts;
    }
    long kain_total_row_access_READ()
    {
       long KAIN_row_access = 0;
       for(int i = 0; i < 64; i++)
            KAIN_row_access += read_row_hits[i].value()+ read_row_misses[i].value() + read_row_conflicts[i].value();
       return KAIN_row_access;
    }
    long kain_total_row_access_WRITE()
    {
       long KAIN_row_access = 0;
       for(int i = 0; i < 64; i++)
            KAIN_row_access += write_row_hits[i].value()+ write_row_misses[i].value() + write_row_conflicts[i].value();
       return KAIN_row_access;
    }

    long kain_total_row_hit()
    {
        long KAIN_row_hit = row_hits.value();
        return KAIN_row_hit;
    }

    long kain_total_row_hit_READ()
    {
        long KAIN_row_hit = 0;
        for(int i = 0; i < 64; i++)
            KAIN_row_hit += read_row_hits[i].value(); 
        return KAIN_row_hit;
    }
    long kain_total_row_hit_WRITE()
    {
        long KAIN_row_hit = 0;
        for(int i = 0; i < 64; i++)
            KAIN_row_hit += write_row_hits[i].value(); 
        return KAIN_row_hit;
    }
    /* Member Functions */
    ...
    void tick()
    {
		...
		kain_cycles_HBM_total++;
//        printf("KAIN Controller tick begin\n");
//        fflush(stdout);
...
//        printf("KAIN Controller tick begin1\n");
//        fflush(stdout);
        /*** 1. Serve completed reads ***/
        if (pending.size() && !r_returnq->full()) {
            Request& req = pending[0];
//        printf("KAIN Controller tick begin2\n");
//        fflush(stdout);
            if (req.depart <= clk) {
                if (req.depart - req.arrive > 1) { // this request really accessed a row
                    read_latency_sum += req.depart - req.arrive;
//        printf("KAIN Controller tick begin6\n");
//        fflush(stdout);
                    channel->update_serving_requests(
                        req.addr_vec.data(), -1, clk);
//        printf("KAIN Controller tick begin7\n");
//        fflush(stdout);
                }
//        printf("KAIN Controller tick begin3\n");
//        fflush(stdout);
                kain_cycle_used++;


				assert(req.mf != NULL);
				unsigned tpc_id = req.mf->get_tpc();
				assert(tpc_id != -1);
				if(Stream1_SM[tpc_id] == true)
					kain_cycles_HBM_app1++;
				else if (Stream2_SM[tpc_id] == true)
					kain_cycles_HBM_app2++;
				else if (Stream3_SM[tpc_id] == true)
					kain_cycles_HBM_app3++;
				else if (Stream4_SM[tpc_id] == true)
					kain_cycles_HBM_app4++;
				else
					assert(0);

                req.callback(req);
//        printf("KAIN Controller tick begin4\n");
//        fflush(stdout);
                pending.pop_front();
//        printf("KAIN Controller tick begin5\n");
//        fflush(stdout);
            }
        }
        else if (r_returnq->full())
        {
            printf("KAIN return queue full\n");
        }
//        printf("KAIN Controller tick Mid1\n");
//        fflush(stdout);

        /*** 2. Refresh scheduler ***/
        refresh->tick_ref();

//        printf("KAIN Controller tick Mid2\n");
//        fflush(stdout);
...
//        printf("KAIN Controller tick Mid3\n");
//        fflush(stdout);
...
//        printf("KAIN Controller tick Mid4\n");
//        fflush(stdout);
...
//        printf("KAIN Controller tick Mid5, addr %0x\n", req->addr);
//            for (int lev = 0; lev < int(T::Level::MAX); lev++)
//                printf(" %5d", req->addr_vec[lev]);
//            printf("\n");
//        fflush(stdout);
        ...
        queue->q.erase(req);
//        printf("KAIN Controller tick end\n");
//        fflush(stdout);
    }
