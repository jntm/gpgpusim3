...
// add by Yongbin Gu
    virtual bool full(Request::Type type, long req_addr) = 0;
};
...
public:
    ...
    bool send(Request req)
    {
        ...
        //    printf("KAIN Stack %d, Channel %d, Rank %d, Bank %d, Row %d, Column %d\n",kain_DRAM_id, req.addr_vec[int(T::Level::Channel)], req.addr_vec[int(T::Level::Rank)], req.addr_vec[int(T::Level::Bank)], req.addr_vec[int(T::Level::Row)], req.addr_vec[int(T::Level::Column)]);
         //   fflush(stdout);
            return true;
        }
        return false;
    ...
    bool full (Request::Type type, long req_addr)
    {
        long addr = req_addr;
        clear_lower_bits(addr, tx_bits);
        int ptr = slice_lower_bits(addr, addr_bits[0]);
        return ctrls[ptr]->full(type);
    }
    ...
    void finish(void) {
        ...
        fprintf(stdout, "maximum_bandwidth is %ld, the rate is %ld, the channel width is %ld, the channel number is %ld\n", long(maximum_bandwidth.value()), long(spec->speed_entry.rate), long(spec->channel_width), long(sz[int(T::Level::Channel)]));
        long dram_cycles = num_dram_cycles.value();
        //printf("KAIN DRAM Cycles %ld\n", dram_cycles);
        long kain_total_byte_RW=0;
        long kain_total_row_hit=0;
        long kain_total_row_access=0;

        long kain_total_byte_READ=0;
        long kain_total_row_hit_READ=0;
        long kain_total_row_access_READ=0;

        long kain_total_byte_WRITE=0;
        long kain_total_row_hit_WRITE=0;
        long kain_total_row_access_WRITE=0;

        for (auto ctrl : ctrls) {
            long read_req = long(incoming_read_reqs_per_channel[ctrl->channel->id].value());
            
            //long kain_read = num_read_requests[ctrl->channel->id].value();
            //long kain_write = num_write_requests[ctrl->channel->id].value();
            //printf("KAIN Read request %ld, write request %ld\n",kain_read,kain_write);
            ctrl->finish(read_req, dram_cycles);
            kain_total_byte_RW +=ctrl->kain_total_byte();
            kain_total_row_access +=ctrl->kain_total_row_access();
            kain_total_row_hit +=ctrl->kain_total_row_hit();

            kain_total_byte_READ +=ctrl->kain_total_byte_READ();
            kain_total_row_access_READ +=ctrl->kain_total_row_access_READ();
            kain_total_row_hit_READ +=ctrl->kain_total_row_hit_READ();

            kain_total_byte_WRITE +=ctrl->kain_total_byte_WRITE();
            kain_total_row_access_WRITE +=ctrl->kain_total_row_access_WRITE();
            kain_total_row_hit_WRITE +=ctrl->kain_total_row_hit_WRITE();
        }
        float BW = ((float)kain_total_byte_RW/((float)dram_cycles/875000000.0))/1000000000.0;
        float BW_READ = ((float)kain_total_byte_READ/((float)dram_cycles/875000000.0))/1000000000.0;
